...
function ServerListLayer:ctor()
    -- 屏幕分辨率
    self._frameSize = cc.Director:getInstance():getOpenGLView():getFrameSize()
    -- 屏幕大小
    self._screenSize = cc.Director:getInstance():getWinSize()
    -- 设计分辨率可视区域大小
...
function ServerListLayer:createEnterGameNode()
	...
	self._lastServer:setPosition( self._screenSize.width/2, 200 )
	...
	self._enterGame:setPosition( self._screenSize.width/2, 100 )
...
function ServerListLayer:createListViewNode()
    ...
    self._listView:setPosition( (self._screenSize.width-240)/2, (self._screenSize.height-48*5)/2 )
