...
const char *g_actordata_table[] = { "actor", "actor_hero", "actor_item" };
const char *g_actordata_table_word[] = { "Actor", "HeroAttr", "Item" };
const char *g_actordata_table_upper[] = { "ACTOR", "HEROATTR", "ITEM" };
const int g_actordata_type[] =        {    0, 	2, 2  };	// 2表示拥有回调函数
...
char *sql_getdatatype( short datatype, char *ptr, char *lkset, int datasize )
{
	...
	case DATATYPE_BIN:
		if ( datasize > 1 )
		{
			sprintf( ptr, "varbinary(%d)", datasize * 4 );
			if ( lkset )
				strcpy( lkset, "NOT NULL DEFAULT ''" );
		}
		break;
...
int actordata_init()
{
	...
	for( tmpgroup = 0; tmpgroup < MAX_ACTORDATA_COUNT; tmpgroup++ )
	{
		...
		while( ( row = mysql_fetch_row( res ) ) )
		{
			...
			if ( g_actordata_struct[tmpgroup].m_sconf[g_actordata_struct[tmpgroup].m_count].data_type != DATATYPE_BIN )
			{
...
int actordata_create()
{
	...
	for( tmpgroup = 0; tmpgroup < MAX_ACTORDATA_COUNT; tmpgroup++ )
	{
		...
		for( tmpi = 0; tmpi < g_actordata_struct[tmpgroup].m_count; tmpi++ )
		{
			...
			if ( g_actordata_struct[tmpgroup].m_sconf[tmpi].data_type == DATATYPE_BIN )
			{ // 字节数组特殊处理
...
int actorsave_create()
{
	...
	for( tmpgroup = 0; tmpgroup < MAX_ACTORDATA_COUNT; tmpgroup++ )
	{
		...
		for( tmpi = 0; tmpi < g_actordata_struct[tmpgroup].m_count; tmpi++ )
		{
			...
			{
				fprintf( fp, "	char szText_%s[MAX_PATH]={0};\r\n", g_actordata_struct[tmpgroup].m_sconf[tmpi].m_name );
			}
		...
		for( tmpi = 0; tmpi < g_actordata_struct[tmpgroup].m_count; tmpi++ )
		{
			...
			if( tmpi > 0 )
				fprintf( fp, "," );
			if ( g_actordata_struct[tmpgroup].m_sconf[tmpi].data_size > 1 && g_actordata_struct[tmpgroup].m_sconf[tmpi].data_type != DATATYPE_CHAR && g_actordata_struct[tmpgroup].m_sconf[tmpi].data_type != DATATYPE_BIN )
			{
		...
		for( tmpi = 0; tmpi < g_actordata_struct[tmpgroup].m_count; tmpi++ )
		{
			...
			if ( g_actordata_struct[tmpgroup].m_sconf[tmpi].data_size > 1 && g_actordata_struct[tmpgroup].m_sconf[tmpi].data_type != DATATYPE_CHAR && g_actordata_struct[tmpgroup].m_sconf[tmpi].data_type != DATATYPE_BIN )
			{
				...
				(g_actordata_struct[tmpgroup].m_sconf[tmpi].data_size > 1 && g_actordata_struct[tmpgroup].m_sconf[tmpi].data_type == DATATYPE_BIN) ||
				g_actordata_struct[tmpgroup].m_sconf[tmpi].data_type == DATATYPE_INT64 )
			{
				fprintf( fp, "'%%s'" );
			}
			else
		...
		for( tmpi = 0; tmpi < g_actordata_struct[tmpgroup].m_count; tmpi++ )
		{
			...
			{
				fprintf( fp, "db_escape((const char *)p%s->%s%s,szText_%s,0)", g_actordata_table_word[tmpgroup], szFix, g_actordata_struct[tmpgroup].m_sconf[tmpi].m_name, g_actordata_struct[tmpgroup].m_sconf[tmpi].m_name, 0 );
			}
			else if ( g_actordata_struct[tmpgroup].m_sconf[tmpi].data_size > 1 && g_actordata_struct[tmpgroup].m_sconf[tmpi].data_type == DATATYPE_BIN )
			{
				struct_getdatatype( g_actordata_struct[tmpgroup].m_sconf[tmpi].data_count, tmpstr, NULL, 0 );
				fprintf( fp, "db_escape((const char *)p%s->%s%s,szText_%s,sizeof(%s)*%d)", g_actordata_table_word[tmpgroup], szFix, g_actordata_struct[tmpgroup].m_sconf[tmpi].m_name, g_actordata_struct[tmpgroup].m_sconf[tmpi].m_name, tmpstr, g_actordata_struct[tmpgroup].m_sconf[tmpi].data_size );
			}
			else
			{
		...
		for( tmpi = 0; tmpi < g_actordata_struct[tmpgroup].m_count; tmpi++ )
		{
			...
			{
				struct_getdatatype( g_actordata_struct[tmpgroup].m_sconf[tmpi].data_count, tmpstr, NULL, 0 );
				fprintf( fp, "		memcpy( p%s->%s%s, row[index++], sizeof(%s)*%d );\r\n", g_actordata_table_word[tmpgroup], szFix, g_actordata_struct[tmpgroup].m_sconf[tmpi].m_name, tmpstr, g_actordata_struct[tmpgroup].m_sconf[tmpi].data_size );
			}
			else if ( g_actordata_struct[tmpgroup].m_sconf[tmpi].data_type == DATATYPE_INT64 )
			{
