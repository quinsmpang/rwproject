...
int struct_init( char *ptab )
{
	...
	while( ( row = mysql_fetch_row( res ) ) )
	{
		...
		strncpy( g_struct[tmpobjid].m_name, row[index++], 32 );
...
char *struct_getdatatype( short datatype, char *ptr, char *lkset )
{
	...
	case DATATYPE_UNINT:
		...
		if( lkset )
			strcpy( lkset, "lua_tointeger" );
		break;
	case DATATYPE_WPARAM:
		strcpy( ptr, "WPARAM " );
		if( lkset )
			strcpy( lkset, "lua_touserdata" );
		break;
	case DATATYPE_LPARAM:
		strcpy( ptr, "LPARAM " );
		if( lkset )
			strcpy( lkset, "lua_touserdata" );
		break;
...
int struct_create_h_ui()
{
	...
	struct_reset( "scui_in_struct" );

	sprintf( szFile, "%s/include/Script_UI_auto.h", szPath );
	fp = fopen( szFile, "wb" );
	if( fp == NULL )
		return -1;

	fprintf( fp, "#ifndef __SCRIPT_UI_auto_H\r\n" );
	fprintf( fp, "#define __SCRIPT_UI_auto_H\r\n" );
	fprintf( fp, "\r\n" );
	fprintf( fp, "#include \"utils.h\"\r\n" );
	fprintf( fp, "\r\n" );

	fprintf( fp, "int LK_Script_Load();\r\n" );
	fprintf( fp, "int LK_Script_Load_DEBUG();\r\n");
	fprintf( fp, "void LK_Script_Register();\r\n" );
	fprintf( fp, "void LK_Script_Function();\r\n" );
	fprintf( fp, "void LK_Script_Exit();\r\n" );
	fprintf( fp, "\r\n" );
...
void struct_create_uireg()
{
	...
	struct_reset( "scui_c_struct" );

	strcpy( szPath, SCUI_C_CREATE_PATH );
	sprintf( szFile, "%s/src/Script_UIReg_auto.cpp", szPath );
	...
	if( linebuf == NULL )
	{
		...
		fprintf( fp, "#include \"ScriptUI.h\"\r\n" );
		fprintf( fp, "#include \"Interface.h\"\r\n" );
		fprintf( fp, "#include \"thread.h\"\r\n" );
		fprintf( fp, "#include \"utils.h\"\r\n" );
		fprintf( fp, "\r\n" );
		fprintf( fp, "extern lua_State* uiL;\r\n" );
	...
	for( tmpi = 0; tmpi < g_struct_count; tmpi++ )
	{
		if( g_struct[tmpi].m_name[0] )
		{
			fprintf( fp, "static int lua_%s( lua_State *uiL );\r\n", g_struct[tmpi].m_name );
		}
	}
	fprintf( fp, "\r\n" );
	fprintf( fp, "void LK_Script_Register()\r\n{\r\n" );
	...
	for( tmpi = 0; tmpi < g_struct_count; tmpi++ )
	{
		if( g_struct[tmpi].m_name[0] )
		{
			...
			if( linebuf )
			{
				sprintf( tmpstr, "static int lua_%s( lua_State *uiL )", g_struct[tmpi].m_name );
			...
			fprintf( fp, "	int num = lua_gettop(uiL);\r\n" );
...
void struct_create_ui()
{
	...
	strcpy( szPath, SCUI_IN_CREATE_PATH );
	sprintf( szFile, "%s/src/Script_UI_auto.cpp", szPath );
	...
	for( tmpi = 0; tmpi < g_struct_count; tmpi++ )
	{
		if( g_struct[tmpi].m_name[0] )
		{
			fprintf( fp, "	lua_getglobal(uiL, \"in_%s\");\r\n", g_struct[tmpi].m_name );
			fprintf( fp, "	_%s_ref = luaL_ref( uiL, LUA_REGISTRYINDEX );\r\n", g_struct[tmpi].m_name );
		}
	}
	fprintf( fp, "}\r\n" );
	fprintf( fp, "\r\n" );
	...
	for( tmpi = 0; tmpi < g_struct_count; tmpi++ )
	{
		if( g_struct[tmpi].m_name[0] )
		{
			fprintf( fp, "	luaL_unref( uiL, LUA_REGISTRYINDEX, _%s_ref );\r\n", g_struct[tmpi].m_name );
		}
	}
	fprintf( fp, "}\r\n" );
	fprintf( fp, "\r\n" );
	...
	for( tmpi = 0; tmpi < g_struct_count; tmpi++ )
	{
		if( g_struct[tmpi].m_name[0] )
		{
			...
			fprintf( fp, "	int nResult;\r\n" );
			fprintf( fp, "	if( uiL == NULL )\r\n" );
			fprintf( fp, "		return -1;\r\n" );
			fprintf( fp, "	lua_rawgeti( uiL, LUA_REGISTRYINDEX, _%s_ref );\r\n", g_struct[tmpi].m_name );
			...
			fprintf( fp, "	lua_call( uiL, %d, 1 );\r\n", g_struct[tmpi].m_arg_count );
			fprintf( fp, "	nResult = (int)lua_tonumber( uiL, -1 );\r\n" );
			fprintf( fp, "	lua_pop( uiL, 1 );\r\n" );
