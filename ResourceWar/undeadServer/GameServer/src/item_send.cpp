...
int item_list( int actor_index )
{
	...
	for ( int tmpi = 0; tmpi < max_itemnum; tmpi++ )
	{
		...
		Value.m_item[Value.m_kindnum].m_type = item_gettype( g_actors[actor_index].item[tmpi].m_kind );
		Value.m_item[Value.m_kindnum].m_num = g_actors[actor_index].item[tmpi].m_num;
		Value.m_item[Value.m_kindnum].m_colorlevel = g_actors[actor_index].item[tmpi].m_color_level;
		Value.m_kindnum++;
...
int item_equip_list( int actor_index, int hero_index )
{
	...
	{ // 如果是英雄
		beginindex = hero_index*MAX_ACTOR_EQUIPNUM;
		max_itemnum = MAX_ACTOR_EQUIPNUM*HERO_MAXCOUNT;
		offsetbase = HEROEQUIP_OFFSETBASE;
		pEquip = g_actors[actor_index].hero_equip;
	}
	else
	{ // 如果是主角
		beginindex = 0;
