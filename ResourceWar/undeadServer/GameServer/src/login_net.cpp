...
int login_init()
{
	...
	{
		printf( "warning! login server is not run!\n" );
	}
...
void login_reconnect( int client_index )
{
	...
	while( 1 )
	{
		if ( platid == 0 || g_PlatInfo[platid].m_allow == 0 )
		{ // 连接默认配置文件里配置的
			g_login_socket = HttpConnect( g_Config.user_host, g_Config.user_port );
		...
		{ // 连接数据库配置的
			g_login_socket = HttpConnect( g_PlatInfo[platid].m_userhost, g_PlatInfo[platid].m_userport );
			write_netlog( "login_connect:host[%s:%d]", g_PlatInfo[platid].m_userhost, g_PlatInfo[platid].m_userport );
			login_settimeout();
		}
		if( g_login_socket != INVALID_SOCKET )
			break;
...
int login_send_proc()
{
	...
	{
//		printf( "Server Is Not Init, login_send_proc Wait...\n" );
#ifdef WIN32
		Sleep( 1000 );
#else
		sleep( 1 );
...
//		fwrite( pstr, sizeof(char), datasize, fp );
	...
	printf( "[thread]Login Proc Exited.\n" );
	return 0;
}
