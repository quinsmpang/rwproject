...
typedef struct _socket_set {
	...
	short			m_isgolden;		// --数据:是否黄金账户
	char			m_isdeep;		// --数据:是否防沉迷
	int				m_point_totle;	// --数据:总充值量
	int				m_expires_in;	// --数据:360超时时间
	char			m_access_token[65];	// --数据:360的token
	char			m_refresh_token[65];// --数据:360的token
	char			m_deviceid[65];	//	--数据:设备ID
	short			m_platid;		// --数据:客户端平台编号
	short			m_language;		// --数据:客户端使用语言
	short			m_invcode;		// --数据:客户端内部标识
	int				m_optype;		// --数据:操作系统(50:android 1:ios)
...
//	short			m_mac_length;
//	short			m_cpu_length;
//	char			m_mac_id[64];	// --数据:客户端的mac地址(用于跟踪和验证)
//	char			m_cpu_id[64];	// --数据:客户端的cpu标识(用于跟踪和验证)
