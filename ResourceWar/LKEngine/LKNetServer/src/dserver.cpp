...
int d_init()
{
	...
	if( fp == NULL )
	{
		printf( "Warnning! Can not open d_list.cfg file, Dserver will allow all ip address.\n" );
...
int d_process( SOCKET fd )
{
	...
	for(;;)
	{
		...
		slen = (short)strlen( recv_buf )+1;

		packsize = 0;
		while( packsize < slen )
		{
			if ( ( writesize = send( fd, recv_buf+packsize, slen-packsize, 0 ) ) <= 0 )
			{
				// 连接断开了(连接重置了)
				d_closesock( fd );
