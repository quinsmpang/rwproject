...
unsigned long g_holdrand = 0;

extern SOCKET_SET *g_sockset;

void netcode_init()
{
	g_holdrand = (unsigned long)time(NULL);
#ifdef _DEBUG
	g_holdrand = 0;
#endif
}

unsigned long netcode_create()
{
//	return 3989106116;
//	return 2006218644;
	return ( ((g_holdrand = g_holdrand * 214013L + 2531011L) >> 16) & 0xffff ) | ( (g_holdrand = g_holdrand * 214013L + 2531011L) & 0xffff0000 );
}

short netcode_getrecvkey( int client_index )
{
//	write_netlog( "recvkey %d", (unsigned short)(( ( g_sockset[client_index].m_recv_key * 214013L + 2531011L) >> 16) & 0xffff ) );
	return (((g_sockset[client_index].m_recv_key = g_sockset[client_index].m_recv_key * 214013L + 2531011L) >> 16) & 0xffff );
}
short netcode_getsendkey( int client_index )
{
//	write_netlog( "sendkey %d", (unsigned short)(( ( g_sockset[client_index].m_send_key * 214013L + 2531011L) >> 16) & 0xffff ) );
	return (((g_sockset[client_index].m_send_key = g_sockset[client_index].m_send_key * 214013L + 2531011L) >> 16) & 0xffff );
}

short netcode_lookrecvkey( int client_index )
{
	return (short)(( ( g_sockset[client_index].m_recv_key * 214013L + 2531011L) >> 16) & 0xffff );
}

short netcode_looksendkey( int client_index )
{
	return (short)(( ( g_sockset[client_index].m_send_key * 214013L + 2531011L) >> 16) & 0xffff );
}
