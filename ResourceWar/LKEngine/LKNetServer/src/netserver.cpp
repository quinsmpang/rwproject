...
int InitGPKServer()
{
	...
	if ( lib.handle == NULL )
	{
		printf( "GPK lib not found[%s]", GPK_DYNAMIC_LIB );
		return -1;
	}
...
int unset_socket( int client_index, char is_remove )
{
	...
	if( client_index >= 0 && client_index < g_max_connection )
	{
		...
		if( is_remove )
		{
			...
			g_sockset[client_index].m_isgolden = 0;
...
//	static char szmsg[MAX_PACKAGE_SIZE+32];
	...
	{
		printf( "g_pCallbackProcessPack Is empty, Wait...\n" );
#ifdef WIN32
		Sleep(1000);
#else
		sleep(1);
...
int work_proc( LPVOID Value )
{
	...
	while(1)
	{
		...
		{
			// Exit
			printf( "[thread]Work Proc Exited.\n" );
			return 0;
		}
...
int net_recv_clientver( int client_index, char *buf, int truesize )
{
	...
	g_sockset[client_index].m_language = language;
	g_sockset[client_index].m_invcode = invcode;
	g_sockset[client_index].m_optype = optype;
...
//	sleep(1);
//	printf("Exiting...[g_bExit:%d-g_bIsDown:%d]\n", g_bExit, g_bIsDown );
//	sleep(1);
}
