...
int netcmd_init()
{
	...
	// 确定每个目标组的个数
	sprintf( szSQL, "select cmdid,cmdstr,structid,target_type,context from netccmd where unuse=0 order by cmdid asc" );
	if( mysql_query( myData, szSQL ) )
	{
		printf( "Query failed (%s)\n", mysql_error(myData) );
		write_gamelog( "%s", szSQL );
...
///////////////////////////////////////////////////////////////////
	...
	// 确定每个目标组的个数
	sprintf( szSQL, "select cmdid,cmdstr,structid,target_type,context,nolua from netscmd where unuse=0 order by cmdid asc" );
	if( mysql_query( myData, szSQL ) )
	{
		printf( "Query failed (%s)\n", mysql_error(myData) );
		write_gamelog( "%s", szSQL );
...
//#endif
		...
		g_netscmd[tmpi].m_nolua = atoi( row[5] );
		tmpi++;
	}
	mysql_free_result( res );
